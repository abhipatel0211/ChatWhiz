...
function Register() {
    ...
    const [Alreadylogedinorregister, setAlreadylogedinorregister] = useState('register');
...
// Rest of your XMLHttpRequest code
            ...
            if (url === "register") {
                await axios.post(`http://localhost:4000/${url}`, { email, password })
                    .then((res) => {
                        //After Register
        ...
        <>
            <div className="bg-blue-100 h-screen flex items-center">
                <form className="w-64 mx-auto" onSubmit={handlesubmit}>
                    {/* <p>Register</p> */}
                    <input value={email} onChange={ (ev) => setemail(ev.target.value)} className="block w-full p-2 mb-2 border" type="text" placeholder="username" required/>
                    ...
                    <div className="text-center mt-2">Already a user ? <button onClick={() => setAlreadylogedinorregister('login')}>Login here</button></div>
                    )}
                    {Alreadylogedinorregister === "login" && (
                    <div className="text-center mt-2">Dont have account ? <button onClick={() => setAlreadylogedinorregister('register')}>Register Here</button></div>
                    )}
                </form>
            </div>
        </>
